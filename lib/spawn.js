...
async function runAgent(agent, task, ctx, signal, onEvent, onInvocation) {
  const cwd = ctx.cwd;
  const args = ["--mode", "json", "-p", "--no-session", "--provider", "openrouter"];
  ...
  try {
    ...
    // Per-agent timeout: if `pi` hangs (LLM outage, infinite tool loop, deadlock),
    // kill the process instead of blocking the pipeline indefinitely. Default 30 min.
    const AGENT_TIMEOUT_MS = ctx.timeoutMs ?? 30 * 60 * 1000;
    ...
    result.exitCode = await new Promise((resolve) => {
      ...
      const onTimeout = () => {
        ...
        wasAborted = true;
        result.errorMessage = `Agent timed out after ${Math.round(AGENT_TIMEOUT_MS / 1000)}s (no output for the full window)`;
        result.stopReason = "timeout";
        proc.kill("SIGTERM");
        setTimeout(() => { if (!proc.killed) proc.kill("SIGKILL"); }, 5000);
      };
      timeoutTimer = setTimeout(onTimeout, AGENT_TIMEOUT_MS);
      // Reset the timer on every stdout/stderr event: timeout only triggers when
      // the process produces *no output at all* for the full window.
      const resetTimer = () => {
        clearTimer();
        timeoutTimer = setTimeout(onTimeout, AGENT_TIMEOUT_MS);
      };
