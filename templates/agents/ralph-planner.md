...
4. **Der Plan muss als letzten Schritt enthalten, dass der Builder committed und pusht:**
   `git add -A && git commit -m "feat: <ticket-title> - implemented" && git push origin HEAD`
...
- Erstelle einen Git-Commit mit dem Plan-Dokument und pushe:
  `git add docs/superpowers/plans/ && git commit -m "docs: plan for ticket <id> - <titel>" && git push origin HEAD`
